...
public abstract class FileChooseInput extends TextFieldWithBrowseButton
{
	...
	{
		VirtualFile file = text == null ? editor.getDefaultValue() : LocalFileSystem.getInstance().findFileByPath(text);
	...
	public static class ExeFile extends FileChooseInput
	{
		...
		{
			return virtualFile.exists() && !virtualFile.isDirectory() && VfsUtil.virtualToIoFile(virtualFile)
					.canExecute();
		}
	...
	public static class OpenOcdHome extends FileChooseInput
	{
		...
		public boolean validateFile(VirtualFile virtualFile)
		{
			...
			if (openOcdBinary == null || openOcdBinary.isDirectory() || !VfsUtil.virtualToIoFile(openOcdBinary)
					.canExecute())
				return false;
