package com.espressif.idf.configurations.debugger.openocd;

import com.espressif.idf.configurations.debugger.EspDebugConfiguration;
import com.espressif.idf.configurations.debugger.Informational;
...
public class OpenOcdComponent
{
	...
	private static final Key<Long> UPLOAD_LOAD_COUNT_KEY = new Key<>(
			EspDebugConfiguration.class.getName() + "#LAST_DOWNLOAD_MOD_COUNT");
	private static final String ERROR_PREFIX = "Error: ";
	private static final String[] IGNORED_STRINGS = { "clearing lockup after double fault", "LIB_USB_NOT_SUPPORTED" };

	private final static String[] FAIL_STRINGS = { "** Programming Failed **", "communication failure",
			"** OpenOCD init failed **" };
	...
	@SuppressWarnings("WeakerAccess")
	@NotNull
	public static GeneralCommandLine createOcdCommandLine(EspDebugConfiguration config, File fileToLoad,
			@Nullable String additionalCommand, boolean shutdown, String targetName) throws ConfigurationException
	{
		...
		GeneralCommandLine commandLine = new PtyCommandLine().withWorkDirectory(ocdBinaryIo.getParentFile())
		...
		if (config.getGdbPort() != DEFAULT_GDB_PORT)
		{
			commandLine.addParameters("-c", "gdb_port " + config.getGdbPort());
		}
		if (config.getTelnetPort() != DEFAULT_TELNET_PORT)
		{
		...
		if (fileToLoad != null)
		{ // Program Command
			String flashCommand = "program_esp_bins <path-to-build-dir> flasher_args.json verify reset";
			CMakeAppRunConfiguration.BuildAndRunConfigurations runConfiguration = config.getBuildAndRunConfigurations(targetName);
			String buildDir = runConfiguration.buildConfiguration.getBuildWorkingDir().getAbsolutePath().replace(File.separatorChar, '/');
			flashCommand = flashCommand.replace("<path-to-build-dir>", buildDir);
			commandLine.addParameters("-c", flashCommand);
		}
	...
	@SuppressWarnings("WeakerAccess")
	public Future<STATUS> startOpenOcd(EspDebugConfiguration config, @Nullable File fileToLoad, String targetName)
			throws ConfigurationException
	{
		if (config == null)
			return new FutureResult<>(STATUS.FLASH_ERROR);
		GeneralCommandLine commandLine = createOcdCommandLine(config, fileToLoad, null, false, targetName);
		...
		try
		{
			...
			RunContentExecutor openOCDConsole = new RunContentExecutor(project, process).withTitle("OpenOCD console")
					.withActivateToolWindow(true).withFilter(new ErrorFilter(project)).withStop(process::destroyProcess,
	...
	private class ErrorFilter implements Filter
	{
		 ...
		 *
		 * @param line         The line to be filtered. Note that the line must contain a line separator at the end.
...
//			else if (vRunFile == null && (text.startsWith(ADAPTER_SPEED) || text.contains("Target halted")))
//			{
//				reset();
